...
public final class ModifiedDietz<T: BinaryFloatingPoint> {
    ...
    public lazy var netCashflowMap: CashflowMap = rawCashflowMap.filter {
        period.start < $0 &&
            $0 <= period.end &&
            epsilon < abs($1)
    }
    ...
    public lazy var orderedCashflowDates: [Date] = netCashflowMap.keys.sorted(by: { $0 < $1 })
    ...
    public lazy var netCashflowTotal: T = netCashflowMap.reduce(0) { $0 + $1.value }
    ...
    public lazy var adjustedNetCashflow: T = netCashflowMap.reduce(0) { accum, flow in
    ...
    public lazy var gainOrLoss: T = marketValue.end - marketValue.start - netCashflowTotal

    /// Average capital over the period.
    public lazy var averageCapital: T = marketValue.start + adjustedNetCashflow
    ...
    public lazy var performance: T = gainOrLoss / averageCapital
}

// MARK: - Internal helpers

extension ModifiedDietz {
    func intervalOfTheFlow(_ transactedAt: Date) -> TimeInterval {
        transactedAt.timeIntervalSince(adjustedPeriod.start)
    }

    func cashFlowAdjustmentFactor(_ transactedAt: Date) -> T {
